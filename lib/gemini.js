...
function isModelUnavailableError(error) {
  const message = [error?.message, error?.response?.error?.message, error?.status, error?.response?.status]
    .filter(Boolean).join(" ");
...
async function runWithGeminiFallback(generate, timeout = 30000) {
  ...
  if (!apiKey) throw new GeminiError("GEMINI_API_KEY is not configured", "UNKNOWN");
  ...
  for (const modelName of modelNames) {
    ...
    while (attempt < MAX_RETRIES) {
      try {
        ...
        // Model doesn't exist — try next model, don't retry
